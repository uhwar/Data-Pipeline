import pandas as pd
from validate import _is_blank
import math
...
def clean(df: pd.DataFrame, validation_report: dict) -> pd.DataFrame:
    ...
    clean_df["product"] = clean_df["product"].str.lower().str.strip()
    ...
    print("[clean] Converted types: date > datetime, quantity > int, unit_price > float")

    # Compute revenue
    clean_df["revenue"] = clean_df["quantity"] * clean_df["unit_price"]
    print("[clean] Computed revenue column")
