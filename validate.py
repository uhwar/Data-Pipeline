...
def validate(df: pd.DataFrame) -> dict:
    ...
    critical_columns = ["order_id", "customer_id", "date", "product", "category", "unit_price"]
    ...
    # errors="coerce" turns all non-convertable into NaN

    # Flag rows where quantity couldn't be converted at all:
    ...
    if bad_status_mask.sum() > 0:
        issues.append(
            f"Column 'status' has {bad_status_mask.sum()} invalid value(s): "
            f"{df.loc[bad_status_mask, 'status'].unique().tolist()}"
        )
    ...
    print(f"[validate] Rows with issues: {len(bad_rows_index)}")
    if issues:
        for issue in issues:
            print(f"  {issue}")
